...
bool due_arm_status = true;  // motors armed status
...
//unsigned long int t_a = 0;
//unsigned long int t_b = 0;
//unsigned long int t_c = 0;
//unsigned long int _t1 = 0;
//unsigned long int _t2 = 0;
float _u0, _u1, _u2;
...
void setup() {
    Timer1.attachInterrupt(refresh_receiver).setPeriod(50000).start();  // refresh receiver values every 50ms - each ppm frame is 20ms
    attachInterrupt(digitalPinToInterrupt(IMU_INTERRUPT_PIN), flight_control, RISING);  // enable data ready interrupt from imu
    due_receiver.configure_receiver();
...
//          Serial.print(_t1);Serial.print("\t");
//          Serial.print(_t2);Serial.print("\n");
