...
a, b = copter.linearisation()
print(a)
continuous_time_system = C.ss(a, b, np.eye(9), 0)
discrete_system = C.c2d(continuous_time_system, Ts=0.1)
...
for k in range(num_simulation_points):
    ...
    copter.fly_simulate([0.001, 0.001, 0], 0.01)  # np.sin(k/50)/100

copter.state = np.array([1] + [0] * 9)  # reset state

for k in range(num_simulation_points):
    linear_black_box_euler = np.vstack((linear_black_box_euler, copter.euler_angles()))
    copter.linear_fly_simulate([0.001, 0.001, 0], 0.01)  # np.sin(k/50)/100
